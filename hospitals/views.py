...
def ver_signos(request):
    if not request.user.is_staff:
        return redirect('login')
    pat = Patient_Info.objects.all()
    d = {'pat':pat}
    return render(request,'ver_signos.html', d)

def borrar_signos(request,pid):
    if not request.user.is_staff:
        return redirect('login')
    patient = Patient_Info.objects.get(id=pid)
    patient.delete()
    return redirect('ver_signos.html')

def editar_signos(request,pid):
    error = ""
    if not request.user.is_authenticated:
        return redirect('login')
    user = request.user
    ...
    if request.method == "POST":
        ...
        try:
            ...
            error = "yes"
    return render(request, 'editar_signos.html', locals())
